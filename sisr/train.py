...
class ScaledDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, input_transform=None, target_transform=None):
    ...
    def __getitem__(self, index):
        input = Image.fromarray(np.rollaxis(self.dataset.data[index], 0, 3)).convert('YCbCr').split()[0]
        target = input.copy()
...
test_set = torchvision.datasets.STL10(root='./data', split='test', download=False)
test_data = ScaledDataset(test_set, input_transform, target_transform)
testing_data_loader = DataLoader(dataset=test_data, num_workers=opt.threads, 
